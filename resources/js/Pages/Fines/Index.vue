   ...
   <AuthenticatedLayout>
      <template #header>
         <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Fines</h2>
            <Alternative :url="route('fines.create')">New fine</Alternative>
         </div>
      </template>

      <div>
         <DataListLayout :footer="fines.meta.last_page > 1">
            <template #header>
               <div>
            ...
            <template #body>
               <table class="w-full text-md text-left text-gray-500 border-b dark:text-gray-400">
                  ...
                  <tbody>
                     <tr v-for="fine in fines.data" :key="fine.id" :class="{
                        'bg-red-100': !fine.paid,
                        'bg-green-100': fine.paid
                     }"
                        ...
                        <td class="py-4 px-6">
                           <Link :href="route('fines.show', fine.id)"
                              class="font-medium mr-3 text-green-600 dark:text-green-500 hover:underline">
                           Show</Link>
                           <Link :href="route('fines.edit', fine.id)"
                              class="font-medium mr-3 text-blue-600 dark:text-blue-500 hover:underline">Edit</Link>
                           <Link :href="route('fines.destroy', fine.id)" method="delete" as="button"
                              class="font-medium text-red-600 dark:text-red-500 hover:underline">Delete
                           </Link>
                        </td>
                     </tr>
                     <tr v-if="fines.data.length <= 0">
                        <td class="py-9 text-xl bg-white text-center font-bold" colspan="6">There is no fine yet</td>
                     </tr>
                  </tbody>
               </table>
            </template>
            <template #footer>
               <div v-if="!mobile" class="font-semibold">
                  <p>Showing from {{ fines.meta.from }} to {{ fines.meta.to }} in total {{ fines.meta.total }}
                     items</p>
               </div>
               <Pagination v-if="fines && mobile" :mobile="mobile" :links="fines.links"></Pagination>
               <Pagination v-if="fines && !mobile" :mobile="mobile" :links="fines.meta.links"></Pagination>
            </template>
         </DataListLayout>
      </div>
   </AuthenticatedLayout>
...
watch(search, debounce(function (value) {
   Inertia.get('/fines', { search: value }, { preserveState: true, replace: true });
}, 300));
