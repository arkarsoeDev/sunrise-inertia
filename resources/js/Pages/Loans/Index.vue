   ...
   <AuthenticatedLayout>
      <template #header>
         <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Loans</h2>
            <Alternative :url="route('loans.create')">New Loan</Alternative>
         </div>
      </template>

      <div>
         <DataListLayout :footer="loans.meta.last_page > 1">
            <template #header>
               <div>
            ...
            <template #body>
               <table class="w-full text-md text-left text-gray-500 border-b dark:text-gray-400">
                  ...
                  <tbody>
                     <tr v-for="loan in loans.data" :key="loan.id" :class="{
                        'bg-red-100': loan.return_date < currentDate && !loan.returned_date,
                        'bg-green-100': loan.returned_date <= loan.return_date && loan.returned_date
                     }"
                        ...
                        <td class="py-4 px-6">
                           {{ loan.returned_date ? formatDate(loan.returned_date) : 'not returned yet' }}
                        </td>
                        <td class="py-4 px-6">
                           ...
                           <Link :href="route('loans.destroy', loan.id)" method="delete" as="button"
            ...
            <template #footer>
               <div v-if="!mobile" class="font-semibold">
                  <p>Showing from {{ loans.meta.from }} to {{ loans.meta.to }} in total {{ loans.meta.total }}
                     items</p>
               </div>
               <Pagination v-if="loans && mobile" :mobile="mobile" :links="loans.links"></Pagination>
               <Pagination v-if="loans && !mobile" :mobile="mobile" :links="loans.meta.links"></Pagination>
            </template>
         </DataListLayout>
      </div>
   </AuthenticatedLayout>
...
watch(search, debounce(function (value) {
   Inertia.get('/loans', { search: value }, { preserveState: true, replace: true });
}, 300));
